...
export const goldModulesQuestions2: GoldQuestion[] = [
  ...
  },
  {
    id: 10425,
    topic: "modules",
    variantOf: "gold-modules-serviceloader-lazy",
    question:
      "次のモジュール構成で、com.example.app/com.example.app.Main を実行した場合の結果として正しいものはどれか。1つ選びなさい。",
    choices: [
      "ImplGreeter created Hello と出力される",
      "created ImplGreeter created Hello と出力される",
      "created ImplGreeter Hello と出力される",
      "com.example.impl パッケージが exports されていないため、実行時に IllegalAccessException がスローされる",
    ],
    correct: [0],
    expected: { kind: "output", stdout: "ImplGreeter created Hello" },
    moduleSetup: {
      main: "com.example.app/com.example.app.Main",
      sources: [
        { module: "com.example.api", path: "module-info.java", content: ["module com.example.api {", "    exports com.example.api;", "}"] },
        ...
        { module: "com.example.app", path: "module-info.java", content: ["module com.example.app {", "    requires com.example.api;", "    uses com.example.api.Greeter;", "}"] },
        {
          ...
          content: [
            ...
            "        System.out.println(loader.findFirst().get().greet());",
            "    }",
            "}",
          ],
        },
    ...
    explanation:
      "ServiceLoader.stream() が返すのは実装のインスタンスではなく ServiceLoader.Provider のストリームで、type() を呼ぶと実装クラスの Class だけを得られます。この時点ではまだコンストラクタは呼ばれないので、ImplGreeter が先に出力されます。実装のインスタンスが作られるのは Provider.get() を呼んだときや、iterator() / findFirst() で要素を取り出したときで、ここで初めて created が出力されます。実装クラスのアノテーションや名前を見て、必要なものだけをインスタンス化するといった選択ができるように、発見とインスタンス化が分けられているのです。実装のパッケージを exports していなくても、Class の名前を調べることや ServiceLoader 経由でのインスタンス化は許されます。",
  },
  {
    id: 10426,
    topic: "modules",
  ...
  },
  {
    ...
    variantOf: "gold-modules-serviceloader",
    question:
      "次のモジュール構成（サービスの実装を提供するモジュールは存在しない）で、com.example.app/com.example.app.Main を実行した場合の結果として正しいものはどれか。1つ選びなさい。",
    choices: [
      "none 0 と出力される",
      "実行時に ServiceConfigurationError がスローされる",
      "実行時に NoSuchElementException がスローされる",
      "com.example.app の module-info.java で、uses に指定したサービスの実装が無いためコンパイルエラーになる",
    ],
    correct: [0],
    expected: { kind: "output", stdout: "none 0" },
    moduleSetup: {
      main: "com.example.app/com.example.app.Main",
      sources: [
        { module: "com.example.api", path: "module-info.java", content: ["module com.example.api {", "    exports com.example.api;", "}"] },
        ...
        { module: "com.example.app", path: "module-info.java", content: ["module com.example.app {", "    requires com.example.api;", "    uses com.example.api.Greeter;", "}"] },
        {
          ...
          content: [
            ...
            "    public static void main(String[] args) {",
            "        ServiceLoader<Greeter> loader = ServiceLoader.load(Greeter.class);",
            "        String s = loader.findFirst().map(Greeter::greet).orElse(\"none\");",
            "        System.out.println(s + \" \" + loader.stream().count());",
            "    }",
            "}",
          ],
        },
    ...
    explanation:
      "uses はサービスを「利用する」という宣言であり、実装がどこかに存在することまでは要求しません。そのため実装を提供するモジュールが 1 つも無くてもコンパイルでき、実行しても例外にはならず、ServiceLoader は空の結果を返します。findFirst() は空の Optional を返すので orElse の none が使われ、stream().count() は 0 です。実装が見つからない状況は「プラグインが 1 つも入っていない」という正常な状態でもありうるので、ServiceLoader は例外ではなく空の結果で表します。一方、uses の宣言自体を書き忘れた場合は ServiceConfigurationError になるので、両者を区別しておきます。",
  },
  ...
  // ------------------------------------------------------------ module-info.java の宣言
  {
    ...
    question: "次のモジュールをコンパイルした場合の結果として正しいものはどれか。1つ選びなさい。",
    choices: [
      "open module の中に opens は書けないため、module-info.java でコンパイルエラーになる",
      "コンパイルに成功する（opens が重複して指定されても問題ない）",
      "open module には exports を書けないため、module-info.java でコンパイルエラーになる",
      "コンパイルに成功するが、実行時に InaccessibleObjectException がスローされる",
    ],
    correct: [0],
    expected: { kind: "compile-error" },
    moduleSetup: {
      sources: [
        {
          ...
          content: ["open module com.example.lib {", "    exports com.example.lib;", "    opens com.example.lib;", "}"],
        },
        { module: "com.example.lib", path: "com/example/lib/Api.java", content: ["package com.example.lib;", "", "public class Api {", "}"] },
      ],
    },
    explanation:
      "open module は、モジュール内のすべてのパッケージを実行時のリフレクションに対して開く宣言です。すべてが既に開かれているので、個別に opens を書くことは許されず、「'opens' only allowed in strong modules」というコンパイルエラーになります（open でない通常のモジュールを strong module と呼びます）。一方、exports は open module でも書けます。open module が開くのは実行時のリフレクションだけで、コンパイル時の参照を許す exports とは別の権限だからです。すべてを開くか、パッケージごとに opens で選ぶかのどちらか一方に決めさせることで、宣言の意味が重複や矛盾なく読み取れるようにしています。",
  },
  {
    id: 10433,
    topic: "modules",
