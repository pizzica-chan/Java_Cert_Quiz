...
export const goldModulesQuestions: GoldQuestion[] = [
  ...
  // ---------------------------------------------------- 依存関係の制約
  {
    ...
    explanation:
      "モジュール間の requires は循環してはならず、order が stock を、stock が order を requires するこの構成は「cyclic dependence」としてコンパイルエラーになります。モジュールの依存関係を一方向の階層に保つことで、下位のモジュールだけを取り出して単独でコンパイル・再利用・テストできるようにしているのです。クラスどうしの相互参照は同じモジュール内であれば問題ありません。循環が生じたら、共通して必要な型を第 3 のモジュールに切り出すか、インタフェースとサービスで依存の向きを一方向にそろえます。",
  },
  {
    id: 10411,
    topic: "modules",
