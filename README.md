...
次の 2 つの試験区分を収録しており、トップ画面で切り替えられます。
...
問題は試験区分ごとのディレクトリ（`src/silver/`、`src/gold/`）に分けています。

## 特徴

- **正解を実機で検証している**（Silver は JDK 11、Gold は JDK 17）
...
- Node.js
- **JDK 11 と JDK 17**（問題の検証に使用。Silver は SE 11、Gold は SE 17 の仕様で検証するため、両方が必要）
